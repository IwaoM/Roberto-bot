...
module.exports = {
  ...
  async execute (oldMember, newMember, client) {
    try {
      logEvent({ name: this.name, description: "A guild member was updated", guild: newMember.guild, member: newMember });
      ...
      logAction({ name: `${this.name} event handling`, guild: newMember.guild, member: newMember });
    } catch (err) {
      logError({
